...
class Algorithm(object):
    ...
    @staticmethod
    def inner_entropy(word, allwords):
        """
        parameters like aggregation func
        return min( right(牡), left(丹花), right(牡丹), left(花) )
        """
        parts = Algorithm.binary_divide(word.text)
        return min(list(chain(*[[allwords[pl].right, allwords[pr].left] for pl, pr in parts])))
...
class Words(object):
    def __init__(self, training_doc, batch_size=10000, max_word=5, min_freq=0.0, min_entropy=0.0, min_aggreg=0.0, min_score=2.0):
        """
            max_word - 一个词最多5个字符长
        """
    ...
    def train_from_candidates_file(self):
        """
            ...
            就避免了重复的计数、统计和计算
        """
        TupleWord = namedtuple("TupleWord", ['text', 'freq', 'left', 'right', 'aggreg', 'inner', 'score'])

        def yield_words_from_file():
            with codecs.open("candidates_statistics.csv", "r", "utf-8") as f:
                for line in f:
                    try:
                        text, freq, left, right, aggreg, inner, score = line.strip().split('\t')
                    except Exception, e:
                        print e
                        print line
                    yield TupleWord(text, float(freq), float(left), float(right), float(aggreg), float(inner), float(score))
        ...
        with codecs.open("good_words.csv", "w", "utf-8") as f:
            ...
            for w in words:
                if w.text not in jieba.dt.FREQ and len(w.text) > 1 and w.aggreg > self.min_aggreg and\
                        ...
                        w.right > self.min_entropy and w.score > self.min_score:
